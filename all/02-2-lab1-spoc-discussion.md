...
>  
```
第一步：初始化IDT，设置时钟中断的中断描述符
第二步：初始化8259中断控制器上的ICW1~ICW4控制关键字
第三步：8253时钟EFLAG=0中断，时钟工作。
```
...
> 先实现一个可以显示字符串的 bootloader,执行工作有硬件设备初始化、分配RAM空间、设置堆栈、跳转到操作系统代码入口点。再实现一个可加载ELF格式的bootloader。
